...
public class PushServer extends HttpServlet {
    ...
    private static String imagePath(int i, int j, HttpServletRequest req, long etag) {
        var lag = RequestParser.parse(req).get(ImageServer.PAR_IMGLAG, 300);
        var delay = RequestParser.parse(req).get(ImageServer.PAR_IMGDELAY, 1000);
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        var etag = ThrottleTool.now();
        if (builder == null) {
            titleText = "Good old HTTP/1.1 download";
        } else {
            var pushRequested = RequestParser.parse(req).get("push", 1) == 1;
            if (pushRequested) {
        ...
        var lag = RequestParser.parse(req).get("lag", 5000);
        var delay = RequestParser.parse(req).get("delay", 0);
    ...
    private void sendHtml(HttpServletRequest req, HttpServletResponse resp, String titleText, long etag, long delay, ThrottleTool.Sleeper sleep) throws IOException {
        try (var html = HtmlTagWriter.htmlWriter(resp.getWriter())) {
    ...
    private void logPush(PushBuilder builder) {
        ...
        msg.append(builder.getMethod()).append(" ").append(builder.getPath()).append("\n");
        for (var hn : builder.getHeaderNames()) {
            msg.append(hn).append(": ").append(builder.getHeader(hn)).append("\n");
        }
        log.pending();
